...
class TrackedObject:
    ...
    def add_frame_roi(self, frame_id, roi):
        ...
        r = ROI(roi.xTopLeft, roi.yTopLeft, roi.xBottomRight, roi.yBottomRight,
                self.objectId)
...
class ROI:
    ...
    def __str__(self):
        return '{} {} {} {}'.format(self.xTopLeft, self.yTopLeft,
                                    self.xBottomRight, self.yBottomRight)
    ...
    def center(self):
        return [[(self.xTopLeft + self.xBottomRight) / 2],
                [(self.yBottomRight + self.yTopLeft) / 2]]

    def reposition(self, center):
        w = (self.xTopLeft - self.xBottomRight)
        h = (self.yBottomRight - self.yTopLeft)
        r = ROI(self.xTopLeft, self.yTopLeft, self.xBottomRight,
                self.yBottomRight, self.objectId)
    ...
    def area(self):
        return abs(self.xTopLeft - self.xBottomRight) * abs(
            self.yTopLeft - self.yBottomRight)
...
class KalmanTrackedObject(TrackedObject):
    ...
    def __init__(self, id, initial_roi: ROI):
        ...
        self.KF = KalmanFilter_ConstantVelocity(
            initial_roi.center())  # KF instance to track this object
        self.color = (int(random.random() * 256),
                      int(random.random() * 256),
                      int(random.random() * 256))
...
class ObjectTracker:
    ...
    def __init__(self, method, firstObjectId=0, distance='hist'):
        """

        Args:
            distance: distance metric to use {hist, pnca}
        """
        self.method = method
        ...
        self.distance_metric = {
            ...
            'pnca': self.pnca_distance
        }[distance]

        self.pnca_model = None
        self.merge_objects_threshold = .76
        self.merge_objects_threshold = .5
        self.merge_tracker_threshold = .95

        if self.distance_metric == self.pnca_distance:
            root_dir = list(os.path.abspath(__file__).rsplit('/')[0:-2])
            path = '/'.join(root_dir + ['proxy-nca', 'model',
                                        'test-aic19-mix-c1c2c3c4c5_final.pt'])
            self.pnca_model = predict.load_model(path)

            self.merge_objects_threshold = .5
            self.merge_tracker_threshold = .75
    ...
    def load_annotated_frame(self, frame: Frame):
        ...
        for r in frame.get_ROIs():
            ...
            tracked_frame.add_ROI(
                ROI(r.xTopLeft, r.yTopLeft, r.xBottomRight, r.yBottomRight,
                    r.objectId))
        ...
        if self.firstFrame:
            ...
            for roi in tracked_frame.get_ROIs():
                if roi.objectId == -1:
                    ...
                    self.trackedObjects[roi.objectId].add_frame_roi(
                        frame.get_id(), roi)
    ...
    def _process_frame_overlap(self, untracked_frame: Frame, overlap_th=0.0,
                               unique_objects=True):
        ...
        for uROI in untracked_frame.get_ROIs():
            ...
            overlapping = np.asarray(
                [uROI.overlap(tROI) for tROI in last_frame_rois])
            ...
            new_tROI = ROI(uROI.xTopLeft, uROI.yTopLeft, uROI.xBottomRight,
                           uROI.yBottomRight, tObjId)
    ...
    # returns a copy of the frame with assigned oject ids if
    # possible (-1 otherwise)
    def _process_frame_kalman(self, untracked_frame: Frame, overlap_th=0.0,
                              unique_objects=True):
        last_frame = self.trackedFrames[untracked_frame.get_id() - 1]
    ...
    # returns a copy of the frame with assigned oject ids if
    # possible (-1 otherwise)
    def _process_frame_optical_flow(self, untracked_frame: Frame,
                                    overlap_th=0.0, unique_objects=True):
        last_frame = self.trackedFrames[untracked_frame.get_id() - 1]
    ...
    def draw_frame(self, frame_number, image):
        ...
        if frame_number in self.trackedFrames:
            for roi in self.trackedFrames[frame_number].get_ROIs():
                ...
                cv2.putText(image, str(roi.objectId), text_pos,
                            cv2.FONT_HERSHEY_SCRIPT_SIMPLEX, .5, (0, 0, 0), 2)
    ...
    def draw_frame_kalman(self, frame_number, image):
        ...
        for roi in frame.get_ROIs():
            color = self.trackedObjects[roi.objectId].color
            roiAux = self.trackedObjects[roi.objectId].track_corrected[
                frame_number]
            ...
            for i in range(frame_number):
                if not corrected:
                    if lastr is None and self.trackedObjects[
                        roi.objectId].track.__contains__(i):
                        lastr = self.trackedObjects[roi.objectId].track[i]
                    elif self.trackedObjects[roi.objectId].track.__contains__(
                        i):
                        ...
                        cv2.line(image, (int(x1), int(y1)), (int(x2), int(y2)),
                                 color, 2)
                        cv2.line(overlay, (int(x1), int(y1)),
                                 (int(x2), int(y2)),
                                 color, 2)
                        lastr = r
                else:
                    if lastr_corrected is None and self.trackedObjects[
                        roi.objectId].track_corrected.__contains__(i):
                        lastr_corrected = \
                            self.trackedObjects[roi.objectId].track_corrected[
                                i]
                    elif self.trackedObjects[
                        roi.objectId].track_corrected.__contains__(i):

                        # Draw trace line
                        r = self.trackedObjects[roi.objectId].track_corrected[
                            i]
    ...
    def compute_mot_metrics(self, other, mtsc_tracker=False):
        ...
        for id, frame in self.trackedFrames.items():
            if self.method == "RegionOverlap" or mtsc_tracker:
                ...
                detROIs = [
                    [r.xTopLeft, r.yTopLeft, r.xBottomRight, r.yBottomRight]
                    for r in frame.get_ROIs()]
    ...
    def removeStaticObjects(self, dist_threshold_percentage=20, width=1920,
                            height=1080):
        ...
        for id1, obj1 in self.trackedObjects.items():
            for id2, obj2 in self.trackedObjects.items():
                if obj1.objectId != obj2.objectId:
                    ...
                    # print(id1, id2, d)
                    d = self.distance_metric(i1, i2)
                    if d > self.merge_objects_threshold:
        ...
        for id1, obj1 in self.trackedObjects.items():
            for id2, obj2 in otherTracker.trackedObjects.items():
                if obj1.objectId != obj2.objectId:
                    ...
                    d = self.distance_metric(i1, i2)
                    # print(id1, id2, d)
                    if d > self.merge_tracker_threshold:
                        toMerge.append((id1, id2, d))
    ...
    def histogram_distance(self, i1, i2):
        hist1 = cv2.calcHist([i1], [0, 1, 2], None, [8, 8, 8],
                             [0, 256, 0, 256, 0, 256])
        hist1 = cv2.normalize(hist1, hist1).flatten()
        hist2 = cv2.calcHist([i2], [0, 1, 2], None, [8, 8, 8],
                             [0, 256, 0, 256, 0, 256])
        hist2 = cv2.normalize(hist2, hist2).flatten()

        d = cv2.compareHist(hist1, hist2, cv2.HISTCMP_CORREL)
        print('histogram: {}'.format(d))
        return d

    def pnca_distance(self, i1, i2):
        i1 = Image.fromarray(i1)
        i2 = Image.fromarray(i2)
        d = predict.distance(self.pnca_model, i1, i2)
        print('pnca: {}'.format(d))
        return d
