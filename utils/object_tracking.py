...
class ROI:
    ...
    def overlap(self, otherROI):
        ...
        if (self.xTopLeft > otherROI.xBottomRight or
            self.xBottomRight < otherROI.xTopLeft):
            return 0

        if (self.yTopLeft > otherROI.yBottomRight or
            self.yBottomRight < otherROI.yTopLeft):
            return 0
...
class KalmanTrackedObject(TrackedObject):
    ...
    def __init__(self, id, initial_roi: ROI):
        ...
        self.KF = KalmanFilter_ConstantAcceleration(
...
class ObjectTracker:
    ...
    def draw_frame_kalman(self, frame_number, image):
        ...
        for roi in frame.get_ROIs():
            ...
            for i in range(frame_number):
                if not corrected:
                    if lastr == None and self.trackedObjects[
                    ...
                    if lastr_corrected == None and self.trackedObjects[
    ...
    def compute_mot_metrics(self, other):
        ...
        for id, frame in self.trackedFrames.items():
            ...
            gtROIs = [[r.xTopLeft, r.yTopLeft, r.xBottomRight, r.yBottomRight]
                      for r in
                      other.trackedFrames[id].get_ROIs()]
