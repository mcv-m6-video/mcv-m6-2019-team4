...
# TODO: remove commented blocks of code by adding if 'valid' if 'test' or similar
...
def MultiTrackSingleCamera(tested_seqs=[3], track_meth='RegionOverlap', detect_meth='ssd512', make_video_track=True,
                           make_video_gt=True, make_video_unfiltered=True, min_conf=.2, parked_threshold=5.0):
...
if __name__ == '__main__':
    ...
    test_seqs = [3]  #, 4]  # training sequences  avoid seq4 as it contains 25 sequences! (too time-consuming)
    ...
    make_vid_track = True
    make_vid_gt = False
    make_vid_unfiltered = True
    plot = False
    min_confidence = .5  # selected after testing coarse grid: 0.2, 0.4, 0.5, 0.7, 0.8
    parked_thresh = 17.5  # from validation on sequence 1 ONLY
    ...
    #         aux_idf1 = MultiTrackSingleCamera(tested_seqs=test_seqs, track_meth=track_methods,
    #                                           detect_meth=detect_method, make_video_track=make_vid_track,
    #                                           make_video_gt=make_vid_gt, min_conf=min_confidence,
    #                                           parked_threshold=parked_thresh)
    ...
    idf1_metrics = MultiTrackSingleCamera(tested_seqs=test_seqs, track_meth=track_methods, detect_meth=detect_method,
                                          make_video_track=make_vid_track, make_video_gt=make_vid_gt,
                                          make_video_unfiltered=make_vid_unfiltered, min_conf=min_confidence,
                                          parked_threshold=parked_thresh)

    print("Printing idf1 metrics for run tests")
    print(idf1_metrics)
